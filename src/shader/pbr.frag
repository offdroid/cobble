// From the Filament design doc https://google.github.io/filament/Filament.html#table_symbols
...
#    ifdef STANDARDMATERIAL_METALLIC_ROUGHNESS_TEXTURE
    vec4 metallic_roughness = texture(sampler2D(StandardMaterial_metallic_roughness_texture, StandardMaterial_metallic_roughness_texture_sampler), vec2(v_Uv));
...
#    ifdef STANDARDMATERIAL_NORMAL_MAP
    mat3 TBN = mat3(T, B, N);
    N = TBN * normalize(texture(sampler2D(StandardMaterial_normal_map, StandardMaterial_normal_map_sampler), vec2(v_Uv)).rgb * 2.0 - 1.0);
#    endif
...
#    ifdef STANDARDMATERIAL_EMISSIVE_TEXTURE
    ...
    emissive.rgb *= texture(sampler2D(StandardMaterial_emissive_texture, StandardMaterial_emissive_texture_sampler), vec2(v_Uv)).rgb;
#    endif
